...
def buy():
    ...
    if request.method == "GET":
        ...
        if user_cash < transaction_value:
            return apology("Insufficient amount of money")
...
def sell():
    ...
    if request.method == "GET":
        user_id = session["user_id"]
        symbols_users = db.execute("SELECT symbol FROM transactions WHERE user_id = :id GROUP BY symbol HAVING SUM(shares) > 0", id = user_id)
        return render_template("sell.html", symbols = [row["symbol"] for row in symbols_users])
    else:
        symbol = request.form.get("symbol")
        shares = int(request.form.get("shares"))
